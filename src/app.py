...
class App(ctk.CTk):
    ...
    def show_occurrence_details(self, occurrence_id):
        ...
        occurrence_data = sheets_service.get_occurrence_by_id(self.credentials, occurrence_id)
        if occurrence_data:
    ...
    def _load_operators_thread(self):
        self.operator_list = sheets_service.get_all_operators(self.credentials)
    ...
    def get_partner_companies(self):
        return sheets_service.get_all_partner_companies(self.credentials)
    ...
    def _fetch_user_profile(self):
        ...
        # A função agora precisa das credenciais para aceder à planilha.
        # ==============================================================================
        self.user_profile = sheets_service.check_user_status(self.credentials, self.user_email)
    ...
    def submit_access_request(self, full_name, username, role, company_name=None):
        ...
        success, message = sheets_service.request_access(self.credentials, self.user_email, full_name, username, internal_role, company_name)
        messagebox.showinfo("Solicitação", message)
        if success:
            self.show_frame("PendingApprovalView")

    def get_pending_requests(self):
        return sheets_service.get_pending_requests(self.credentials)

    def update_user_access(self, email, new_status):
        sheets_service.update_user_status(self.credentials, email, new_status)
    ...
    def get_all_occurrences(self, status_filter=None, role_filter=None, search_term=None):
        return sheets_service.get_all_occurrences(self.credentials, status_filter, role_filter, search_term)
    ...
    def save_occurrence_status_changes(self, changes):
        ...
        for occ_id, new_status in changes.items():
            sheets_service.update_occurrence_status(self.credentials, occ_id, new_status)
        messagebox.showinfo("Sucesso", f"{len(changes)} alterações foram salvas.")
        self.frames["AdminDashboardView"].load_all_occurrences()

    def get_all_users(self):
        return sheets_service.get_all_users(self.credentials)

    def update_user_role(self, email, new_role):
        sheets_service.update_user_role(self.credentials, email, new_role)
    ...
    def _submit_simple_call_thread(self, form_data):
        success, message = sheets_service.register_simple_call_occurrence(self.credentials, self.user_email, form_data)
    ...
    def _submit_full_occurrence_thread(self, title, testes):
        success, message = sheets_service.register_full_occurrence(self.credentials, self.user_email, title, testes)
