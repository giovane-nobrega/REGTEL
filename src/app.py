...
from views.admin_dashboard_view import AdminDashboardView # Mantemos a importação do AdminDashboardView (agora o de cards)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # CORREÇÃO: Removido fill="both" e expand=True do pack()
        container.pack(fill="both", expand=True) 

        # Configura o container para usar grid e permitir que as frames filhas se expandam
        ...
        view_classes = (
            ...
            AdminDashboardView, # Agora é o dashboard de cards
            HistoryView, RegistrationView, SimpleCallView,
            EquipmentView,
            AccessManagementView, # Nova View
            UserManagementView    # Nova View
        )
        ...
        for F in view_classes:
            ...
            # CORREÇÃO: Usar grid() em vez de place() para as frames filhas no container
            # Isso garante que as views se expandam corretamente dentro do container
            frame.grid(row=0, column=0, sticky="nsew") # Posiciona todas as views no mesmo local, mas apenas uma é visível por vez
    ...
    def show_frame(self, page_name, from_view=None):
        ...
        # CORREÇÃO: Ao usar grid() para as frames, o tkraise() ainda é usado para trazer a frame para a frente
        frame.tkraise()
    ...
    def navigate_based_on_status(self):
        ...
        if status == "approved":
            ...
            if main_group == "67_TELECOM" and sub_group == "SUPER_ADMIN":
                # Direciona para o novo AdminDashboardView (o de cards)
    ...
    def update_user_access(self, email, new_status):
        ...
        if new_status == 'approved':
            ...
            NotificationPopup(self, message=f"O acesso para {email} foi rejeitado.", type="info",
                              bg_color_success="green", text_color_success="white",
                              bg_color_info="gray", text_color_info="white")
        # Após a atualização, recarrega a lista na tela de gerenciamento de acessos se ela estiver ativa
        if self.current_frame == "AccessManagementView":
            self.frames["AccessManagementView"].load_access_requests()
        # Também recarrega o dashboard para atualizar a contagem de solicitações pendentes
        if self.current_frame == "AdminDashboardView":
            self.frames["AdminDashboardView"]._load_pending_access_requests_count()
    ...
    def save_occurrence_status_changes(self, changes):
        ...
        if success:
            ...
            NotificationPopup(self, message=f"Os status de {len(changes)} ocorrência(s) foram salvos com sucesso.", type="success",
                              bg_color_success="green", text_color_success="white",
                              bg_color_info="gray", text_color_info="white")
            # Força o recarregamento do dashboard para atualizar a contagem de ocorrências pendentes
            if self.current_frame == "AdminDashboardView":
                self.frames["AdminDashboardView"]._load_pending_occurrences_count()
    ...
    def update_user_profile(self, changes: dict):
        ...
        if success:
            ...
            NotificationPopup(self, message=f"{len(changes)} perfil(is) de usuário foram atualizados com sucesso.", type="success",
                              bg_color_success="green", text_color_success="white",
                              bg_color_info="gray", text_color_info="white")
            # Após a atualização, recarrega a lista na tela de gerenciamento de usuários se ela estiver ativa
            if self.current_frame == "UserManagementView":
                self.frames["UserManagementView"].load_all_users(force_refresh=True)
            # Também recarrega o dashboard para atualizar a contagem de utilizadores ativos
            if self.current_frame == "AdminDashboardView":
                self.frames["AdminDashboardView"]._load_active_users_count()
        else:
    ...
    def update_occurrence_status_from_history(self, occurrence_id, new_status):
        ...
        if success:
            ...
            if "HistoryView" in self.frames:
                self.frames["HistoryView"].load_history()
            # Se o AdminDashboardView estiver aberto, força o recarregamento da contagem de ocorrências pendentes
            if self.current_frame == "AdminDashboardView":
                self.frames["AdminDashboardView"]._load_pending_occurrences_count()
        else:
