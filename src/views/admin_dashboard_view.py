...
class AdminDashboardView(ctk.CTkFrame):
    ...
    def setup_analysis_tab(self):
        ...
        # Container principal com rolagem para toda a aba
        scrollable_container = ctk.CTkScrollableFrame(
            self.analysis_tab, fg_color="transparent")
        scrollable_container.pack(fill="both", expand=True)

        # Frame para os filtros
        filter_frame = ctk.CTkFrame(scrollable_container)
        filter_frame.pack(fill="x", padx=5, pady=5)

        ctk.CTkLabel(filter_frame, text="Filtrar por Status:").pack(
            side="left", padx=(10, 5))
        self.status_filter_menu = ctk.CTkOptionMenu(filter_frame, values=[
                                                    "Todos", "Registrado", "Em Análise", "Aguardando Terceiros", "Resolvido", "Cancelado"], command=self.apply_filters)
        self.status_filter_menu.pack(side="left", padx=5)

        ctk.CTkLabel(filter_frame, text="Filtrar por Grupo:").pack(
            side="left", padx=(20, 5))
        self.role_filter_menu = ctk.CTkOptionMenu(filter_frame, values=[
                                                  "Todos", "partner", "prefeitura"], command=self.apply_filters)
        self.role_filter_menu.pack(side="left", padx=5)

        # Frame para os gráficos e estatísticas
        analysis_content_frame = ctk.CTkFrame(
            scrollable_container, fg_color="transparent")
        analysis_content_frame.pack(fill="both", expand=True, pady=10)
        analysis_content_frame.grid_columnconfigure((0, 1), weight=1)
        analysis_content_frame.grid_rowconfigure(1, weight=1)

        # Estatísticas
        self.stats_label = ctk.CTkLabel(analysis_content_frame, text="Estatísticas: Carregando...",
                                        justify="left", font=ctk.CTkFont(size=14, weight="bold"))
        self.stats_label.grid(row=0, column=0, columnspan=2,
                              sticky="w", padx=10, pady=(10, 5))

        # Gráficos
        self.status_chart_frame = ctk.CTkFrame(analysis_content_frame)
        self.status_chart_frame.grid(
            row=1, column=0, sticky="nsew", pady=5, padx=(5, 5))

        self.operator_chart_frame = ctk.CTkFrame(analysis_content_frame)
        self.operator_chart_frame.grid(
            row=1, column=1, sticky="nsew", pady=5, padx=(5, 5))

    def apply_filters(self, _=None):
        """Aplica os filtros selecionados e atualiza os gráficos."""
        status = self.status_filter_menu.get()
        role = self.role_filter_menu.get()

        self.stats_label.configure(text="Estatísticas: Calculando...")

        if self.status_chart_canvas:
            self.status_chart_canvas.get_tk_widget().destroy()
        if self.operator_chart_canvas:
            self.operator_chart_canvas.get_tk_widget().destroy()
        for widget in self.status_chart_frame.winfo_children():
            widget.destroy()
        for widget in self.operator_chart_frame.winfo_children():
            widget.destroy()
        ctk.CTkLabel(self.status_chart_frame,
                     text="Carregando...").pack(expand=True)
        ctk.CTkLabel(self.operator_chart_frame,
                     text="Carregando...").pack(expand=True)

        threading.Thread(target=self._apply_filters_thread,
                         args=(status, role), daemon=True).start()

    def _apply_filters_thread(self, status, role):
        """Busca os dados filtrados em segundo plano."""
        filtered_list = self.controller.get_all_occurrences(
            status_filter=status, role_filter=role)
        self.after(0, self._populate_analysis_results, filtered_list)

    def _populate_analysis_results(self, results):
        """Preenche a aba de análise com os gráficos e estatísticas."""
        for widget in self.status_chart_frame.winfo_children():
            widget.destroy()
        for widget in self.operator_chart_frame.winfo_children():
            widget.destroy()
        ...
        if not results:
            self.stats_label.configure(
                text="Estatísticas: 0 ocorrências encontradas.")
            ctk.CTkLabel(self.status_chart_frame,
                         text="Nenhum dado para exibir.").pack(expand=True)
            ctk.CTkLabel(self.operator_chart_frame,
                         text="Nenhum dado para exibir.").pack(expand=True)
            return

        total = len(results)
        status_counts = {"Registrado": 0, "Em Análise": 0,
                         "Aguardando Terceiros": 0, "Resolvido": 0, "Cancelado": 0}
        operator_counts = {}
        for item in results:
        ...
        self.update_chart(self.status_chart_frame, status_counts,
                          "Ocorrências por Status", "Nº de Ocorrências", "status_chart_canvas")
        self.update_chart(self.operator_chart_frame, operator_counts,
                          "Menções por Operadora", "Nº de Menções", "operator_chart_canvas")

    def update_chart(self, frame, data, title, ylabel, canvas_attr_name):
        """Função genérica para criar ou atualizar um gráfico de barras."""
        if hasattr(self, canvas_attr_name) and getattr(self, canvas_attr_name):
            getattr(self, canvas_attr_name).get_tk_widget().destroy()
    ...
    def setup_access_tab(self):
        ...
        self.pending_users_frame = ctk.CTkScrollableFrame(
            self.access_tab, label_text="Carregando...")
    ...
    def setup_users_tab(self):
        ...
        self.all_users_frame = ctk.CTkScrollableFrame(
            self.users_tab, label_text="Carregando...")
    ...
    def setup_occurrences_tab(self):
        ...
        self.all_occurrences_frame = ctk.CTkScrollableFrame(
            self.occurrences_tab, label_text="Carregando...")
    ...
    def load_access_requests(self):
        self.pending_users_frame.configure(
            label_text="Carregando solicitações...")
    ...
    def _populate_all_users(self, all_users_list):
        ...
        if not all_users_list:
            self.all_users_frame.configure(
                label_text="Nenhum usuário encontrado.")
            return
    ...
    def save_role_changes(self):
        ...
        if not changes_to_save:
            messagebox.showinfo("Nenhuma Alteração",
                                "Nenhum perfil de usuário foi alterado.")
            return
    ...
    def load_all_occurrences(self):
        self.all_occurrences_frame.configure(
            label_text="Carregando ocorrências...")
        threading.Thread(
    ...
    def _populate_all_occurrences(self, all_occurrences_list):
        ...
        for item in all_occurrences_list:
            ...
            ctk.CTkLabel(card, text=f"Registrado por: {email} em {date}", anchor="w", text_color="gray60").grid(
