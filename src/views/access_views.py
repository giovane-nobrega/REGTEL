...
from tkinter import messagebox # Usado para exibir caixas de mensagem de alerta/erro
...
from builtins import bool, hasattr, super, len, range # CORRIGIDO: Importa built-ins explicitamente para satisfazer o Pylance
...
class RequestAccessView(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        self.role_combobox.set("Prefeitura") # Define o valor inicial padrão para o vínculo principal
        self._on_role_selected("Prefeitura") # Chama a função para configurar os campos dependentes
    ...
    def _on_role_selected(self, selected_role):
        ...
        self._validate_company_live() # Revalida os campos após a mudança de vínculo.
    ...
    def on_show(self):
        ...
        self.role_combobox.set("Prefeitura") # Define o valor padrão para o vínculo
        self._on_role_selected("Prefeitura") # Aciona a lógica de visibilidade de campos dependentes
    ...
    def submit(self):
        ...
        if main_group == "PARTNER":
            # Mapeia a empresa selecionada para o subgrupo correspondente
            ...
            sub_group = self.subgroup_67_combobox.get() # Subgrupo é o selecionado no combobox
...
class PendingApprovalView(ctk.CTkFrame):
    ...
    def on_show(self):
        ...
        if not self.is_running or (self.checker_thread and not self.checker_thread.is_alive()):
            ...
            self.checker_thread = threading.Thread(target=self._check_status_thread, daemon=True) # Thread daemon para fechar com a aplicação
    ...
    def _check_status_thread(self):
        ...
        while self.is_running: # Loop continua enquanto a flag 'is_running' for True
            ...
            if status == "approved":
                ...
                self.controller.user_profile = user_profile # Atualiza o perfil no controlador principal
                self.controller.after(0, self.controller.navigate_based_on_status) # Navega para o próximo ecrã
                ...
                self.controller.after(0, self.controller.perform_logout) # Realiza logout
            else:
    ...
    def check_status_now(self):
        ...
        if self.is_running:
            ...
            if self.checker_thread and self.checker_thread.is_alive():
                self.checker_thread.join(timeout=1) # Espera no máximo 1 segundo pela thread
