# ==============================================================================
# ARQUIVO: src/views/access_views.py
# DESCRIÇÃO: Contém as classes de interface para o fluxo de solicitação
#            e aprovação de acesso de novos usuários.
...
class RequestAccessView(ctk.CTkFrame):
    """
    Tela para novos usuários solicitarem acesso ao sistema REGTEL.
    Permite que o usuário preencha seus dados pessoais e de vínculo.
    """
    def __init__(self, parent, controller):
        ...
        subtitle = ctk.CTkLabel(center_frame, text="Seu e-mail não está registrado. Por favor, preencha seus dados para solicitar o acesso.",
    ...
    def _validate_username_live(self, event=None):
        ...
        if is_valid:
            self.username_entry.configure(border_color=self.default_border_color)
            self.username_validation_label.configure(text="✔️ Nome de usuário válido", text_color="green")
        else:
            self.username_entry.configure(border_color="red")
            self.username_validation_label.configure(text=f"❌ Nome de usuário inválido: {message}", text_color="red")
...
class PendingApprovalView(ctk.CTkFrame):
    """
    Tela exibida enquanto o acesso do usuário está pendente de aprovação por um administrador.
    ...
    def __init__(self, parent, controller):
        ...
        self.status_label = ctk.CTkLabel(center_frame, text="Verificando o status...",
    ...
    def _check_status_thread(self):
        while self.is_running:
            self.controller.after(0, lambda: self.status_label.configure(text="Verificando o status..."))
            ...
            if status == "approved":
                ...
                self.is_running = False
                self.controller.after(0, lambda: messagebox.showerror("Acesso Negado", "Sua solicitação de acesso foi rejeitada. Por favor, entre em contato com o administrador."))
    ...
    def check_status_now(self):
        if self.is_running:
            ...
            self.is_running = True
            self.status_label.configure(text="Verificando o status agora...")
            self.checker_thread = threading.Thread(target=self._check_status_thread, daemon=True)
            self.checker_thread.start()
