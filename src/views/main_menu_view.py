...
class MainMenuView(ctk.CTkFrame):
    """A tela do menu principal, com botões que mudam com base no perfil do utilizador."""
    def __init__(self, parent, controller):
        ...
        # --- Botão de Admin ---
        self.admin_dashboard_button = ctk.CTkButton(
            self.buttons_frame, 
        ...
        # --- Botões Comuns ---
        self.history_button = ctk.CTkButton(self.buttons_frame, text="Ver o Meu Histórico de Ocorrências", command=lambda: self.controller.show_frame("HistoryView"), height=45, width=350, font=ctk.CTkFont(size=14))
        self.history_button.pack(pady=8, padx=20, fill="x")
        
        self.logout_button = ctk.CTkButton(self.buttons_frame, text="Logout (Trocar de utilizador)", command=self.controller.perform_logout, height=45, width=350, font=ctk.CTkFont(size=14), fg_color="#D32F2F", hover_color="#B71C1C")
    ...
    def update_user_info(self, email, username):
        """Atualiza o e-mail e o nome de utilizador exibidos."""
    ...
    def update_buttons(self, role):
        """Mostra ou oculta os botões com base no perfil do utilizador."""
        # Oculta todos os botões específicos de perfil primeiro
        for widget in (self.admin_dashboard_button, self.partner_button, self.prefeitura_call_button, self.prefeitura_equip_button):
            widget.pack_forget()
        
        # Mostra os botões corretos com base no perfil
        if role == "admin":
            self.admin_dashboard_button.pack(pady=8, padx=20, fill="x", before=self.history_button)
        elif role == "partner":
            self.partner_button.pack(pady=8, padx=20, fill="x", before=self.history_button)
