...
class RegistrationView(ctk.CTkFrame):
    """Tela para o registo detalhado de ocorrências (Parceiro)."""
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(test_list_frame, text="3. Testes a Serem Registados (mínimo 2)", font=ctk.CTkFont(size=16, weight="bold")).pack(anchor="w", padx=10, pady=(10,5))
        ...
        self.submit_button = ctk.CTkButton(final_buttons_frame, text="Registar Ocorrência Completa", command=self.submit, height=40)
    ...
    def delete_test(self, index):
        """Remove um teste da lista de evidências."""
        if messagebox.askyesno("Confirmar Exclusão", "Tem a certeza que deseja excluir este teste?"):
            self.controller.testes_adicionados.pop(index)
            self._update_test_display_list()

    def edit_test(self, index):
        """Preenche o formulário para editar um teste existente."""
    ...
    def submit(self):
        ...
        title = self.entry_ocorrencia_titulo.get()
    ...
    def set_submitting_state(self, is_submitting):
        ...
        if is_submitting:
            ...
            self.submit_button.configure(state="normal", text="Registar Ocorrência Completa")