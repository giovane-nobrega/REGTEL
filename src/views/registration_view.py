...
class RegistrationView(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(test_entry_frame, text="Horário do Teste").grid(
            row=1, column=0, sticky="w", padx=10, pady=(5, 0))
        self.entry_teste_horario = ctk.CTkEntry(
            test_entry_frame, placeholder_text="Ex: 16:05")
        ...
        ctk.CTkLabel(test_entry_frame, text="Observações (opcional)").grid(
    ...
    def add_or_update_test(self):
        ...
        if not all([horario, num_a, op_a, num_b, op_b, status]):
            messagebox.showerror(
                "Erro de Validação", "Todos os campos do teste de ligação (exceto Observações) são obrigatórios.")
            return
