...
class RegistrationView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # A linha da lista de testes expande
        self.grid_rowconfigure(2, weight=1)

        # --- Widgets ---
        main_occurrence_frame = ctk.CTkFrame(self)
        main_occurrence_frame.grid(
        ...
        self.entry_teste_horario.bind(
            "<FocusOut>", self._validate_and_format_horario)
        ...
        self.entry_op_a = AutocompleteEntry(
            test_entry_frame, placeholder_text="Digite para buscar...")
        ...
        self.entry_op_b = AutocompleteEntry(
            test_entry_frame, placeholder_text="Digite para buscar...")
        ...
        # --- ALTERAÇÃO AQUI ---
        ctk.CTkLabel(test_entry_frame, text="Descrição do Problema (obrigatório)").grid(
            row=5, column=0, columnspan=3, sticky="w", padx=10, pady=(5, 0))
        self.entry_teste_obs = ctk.CTkEntry(
            test_entry_frame, placeholder_text="Descreva o problema detalhadamente (ex: a ligação caiu após 5 segundos)")
    ...
    def on_show(self):
        ...
        if role == 'partner':
            self.test_list_label.configure(
                text="3. Testes a Serem Registrados (mínimo 3)")
        else:
            self.test_list_label.configure(
                text="3. Testes a Serem Registrados")
    ...
    def add_or_update_test(self):
        ...
        if not all([horario, num_a, op_a, num_b, op_b, status, obs]):
            messagebox.showerror(
                "Erro de Validação", "Todos os campos do teste, incluindo a Descrição do Problema, são obrigatórios.")
            return
    ...
    def _update_test_display_list(self):
        ...
        for index, teste in enumerate(self.controller.testes_adicionados):
            ...
            # --- ALTERAÇÃO AQUI ---
            if teste['obs']:
                ctk.CTkLabel(info_frame, text=f"Descrição: {teste['obs']}", anchor="w", font=ctk.CTkFont(
    ...
    def _validate_and_format_horario(self, event=None):
        ...
        if not digits.isdigit() or len(digits) != 4:
            messagebox.showerror(
                "Formato Inválido", "Por favor, insira o horário com 4 dígitos no formato HHMM (ex: 1430).")
            self.entry_teste_horario.after(
                0, lambda: self.entry_teste_horario.delete(0, 'end'))
            return
        try:
            hora = int(digits[0:2])
            ...
            messagebox.showerror(
                "Horário Inválido", f"O horário '{horario_str}' não é válido. Use o formato HHMM.")
            self.entry_teste_horario.after(
                0, lambda: self.entry_teste_horario.delete(0, 'end'))
            return
